...
public class CommandGenerator : IGenerator
{
	...
	{
		if (registry.Commands == null) return;
		...
		foreach (Command command in registry.Commands.Command)
		{
			...
			writer.WriteLine(@$"public static {returnType} {name}({parameters.Select((x, i) => new { x, index = i }).OrderBy(x => x.x.isOptional).ThenBy(x => x.index)
				.Select(x => (x.x.isOut ? "out " : "") + (remove(x.x.type) ? x.x.type[..^1] : x.x.type) + " " + x.x.name + (x.x.isOptional ? " = default" : "")).JoinStrings()})");
			writer.WriteLine("{");
			writer.Indent();
			if (returnType != "void") writer.WriteLine($"return {name}_ptr({parameters.Select(x => (x.isOut ? "out " : "") + (remove(x.type) ? "&" : "") + x.name).JoinStrings()});");
			else writer.WriteLine($"{name}_ptr({parameters.Select(x => (x.isOut ? "out " : "") + (remove(x.type) ? "&" : "") + x.name).JoinStrings()});");
			writer.Unindent();
			writer.WriteLine("}");
			writer.WriteLine();
		...
		{
			return name.Contains("CreateInfo") || name.Contains("BeginInfo") || name.Contains("AllocateInfo");
		}
