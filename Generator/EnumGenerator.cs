...
public class EnumGenerator : IGenerator
{
	...
	private static string GetName(string value, string enumPrefix, bool removeExt = true)
	{
		...
		if (value.IndexOf(enumPrefix, StringComparison.Ordinal) != 0) return value;
	...
	{
		var grouped = registry.Enums.GroupBy(x => x.Type ?? "other").ToDictionary(x => x.Key, x => x.ToList());

		if (grouped.TryGetValue("enum", out var enumsGroup)) GenerateEnums(outputDir, enumsGroup, registry);
		if (grouped.TryGetValue("bitmask", out var bitmaskGroup)) GenerateFlags(outputDir, bitmaskGroup, registry);

		CodeWriter writer = new("Fireburst", "System");

		foreach (Type type in registry.Types.Type.Where(x => x.Category is "bitmask" or "enum"))
		{
			if (type.Alias != null)
			{
